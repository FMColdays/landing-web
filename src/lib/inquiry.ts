export interface Inquiry { name: string; business: string; service: string; message?: string; }
export function createInquiry(input: Inquiry, phone: string, email: string) {
 ...
 return { message, whatsappUrl: `https://wa.me/${phone}?text=${encodeURIComponent(message)}`, emailUrl: `mailto:${email}?subject=${encodeURIComponent('Cotización de página web')}&body=${encodeURIComponent(message)}` };
}
